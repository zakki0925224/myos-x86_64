...
extern "x86-interrupt" fn debug_handler(stack_frame: InterruptStackFrame) {
    ...
    if let Some(dwarf) = task::scheduler::get_current_user_task_dwarf() {
...
extern "x86-interrupt" fn general_protection_fault_handler(stack_frame: InterruptStackFrame) {
    ...
    if task::scheduler::debug_current_user_task() {
        task::scheduler::exit_current_user_task(122);
    }
...
extern "x86-interrupt" fn page_fault_handler(
    ...
    if task::scheduler::debug_current_user_task() {
        task::scheduler::exit_current_user_task(123);
    }
